...
<body>
  <div class="container">
    ...
    <section class="day-card" id="day-1">
      ...
      [Blockquote]
      [Diagram image if applicable — wrapped in a clickable link that opens the full-size PNG in a new tab]
      [Hands-on Activity]
    </section>
    [...repeat for each day]
    ...
    <div class="related-docs">
      ...
      <table>
        ...
        <tbody>
          <tr><td><strong>Published HTML</strong></td><td><code>outputs/[topic-slug]/07-[topic-slug]-module-content.html</code> — generated by Skill 7</td></tr>
          <tr><td><strong>Capstone Project Brief</strong></td><td><code>outputs/[topic-slug]/08-[topic-slug]-capstone.md</code> — generated by Skill 8</td></tr>
          <tr><td><strong>Hands-On Activities</strong></td><td><code>outputs/[topic-slug]/09-[topic-slug]-hands-on-activities.md</code> — generated by Skill 9</td></tr>
        </tbody>
      </table>
    </div>
...
- Gantt image at full width with `max-width: 100%; height: auto;`
- "Training Timeline" label above the image
- Gantt image is also clickable — wrap in `<a href="images/gantt.png" target="_blank">`
...
### Diagram image sizing and click behaviour
- Diagrams in day cards should render at **full card width** so they are large enough to read (use `width: 100%; height: auto;` on the `<img>`)
- Wrap each diagram image in a `<a href="images/diagram-N.png" target="_blank">` link so learners can click to open the full-size PNG in a new browser tab
- Add a small "click to enlarge" hint — either a CSS cursor change or a subtle label
...
- Render to PNG using the mermaid.ink API (`https://mermaid.ink/img/{base64}`):
  ...
  - Append `?width=1200` to produce a larger, higher-resolution image so text is readable
  - **Do NOT append `?theme=dark`** — use the default light theme
